...
namespace coen79_lab2 {
    ...
    {
        public:
            rand_gen(int seed = 1, int multiplier = 1, int increment = 1, int modulus = 1); // constructor with parameters defaulting to 1
            void set_seed(int newSeed); // inserts new seed into the pseudo random generator
            int next(); // returns next pseudo random number and sets that to the new seed
            void print_info() const; // prints out the info
        private:
            int seed;
            int multiplier;
            int increment;
