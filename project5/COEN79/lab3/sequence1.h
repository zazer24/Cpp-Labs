...
namespace coen79_lab3 {
    ...
    class sequence {
        ...
        public:
            ...
            void start(); // returns current to the start of the sequence
            void end(); // moves current to the end of the sequence
            void last(); // reduces the sequence by one
            void advance(); // iterates forwards by one
            void retreat(); // iterates backwards by one
            void insert(const value_type& entry); /* inserts value into the 
                                                    sequence before current */
            void attach(const value_type& entry);  /* inserts value into the 
                                                    sequence after current */
            void remove_current();
            void insert_front(const value_type& entry); /* applies insert function but uses 
                                                        zero index instead of current */
            void attach_back(const value_type& entry); /* applies attach function but uses 
                                                        used - 1 index instead of current */
            void remove_front(const value_type& entry);
            void operator +=(const sequence& rhs); /* overloads += operator to enable
                                                         sequence addition with itself */
            size_type size() const;
            bool is_item() const; // asserts thats the array index in question is valid
            value_type current() const;
            value_type operator[](int index) const; /* overloads [] operator to 
                                                        allow sequence indexing */
            double mean() const; 
            double standardDeviation() const;
        private:
            size_type used; // number of values in the sequence
            size_type index; // current index
            value_type arr[CAPACITY]; // max number of values allowed in the sequence
    };
sequence operator +(const sequence& lhs, const sequence& rhs); /* overloads the + operator to 
                                                                    allow sequence addition */
